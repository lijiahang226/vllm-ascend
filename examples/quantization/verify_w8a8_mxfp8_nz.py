...
"""Compare W8A8 MXFP8 linear ND/NZ paths on A5, without vLLM or a model.

Run: python verify_w8a8_mxfp8_nz.py --device 0
NZ conversion transposes FP8 weights, makes them contiguous, then casts.
"""
...
def main():
    ...
    parser.add_argument("--scale-alg", type=int, choices=(0, 1), default=0)
    ...
    xq, xs = torch_npu.npu_dynamic_mx_quant(x, dst_type=torch.float8_e4m3fn, scale_alg=args.scale_alg)
    wq, ws = torch_npu.npu_dynamic_mx_quant(w, dst_type=torch.float8_e4m3fn, scale_alg=args.scale_alg)
    assert wq.dtype == torch.float8_e4m3fn and tuple(wq.shape) == (args.n, args.k)
    assert tuple(ws.shape) == (args.n, args.k // 64, 2)
    describe("xq", xq)
    describe("x_scale", xs)
    describe("FP8_weight", wq)

    # Match the W8A8 MXFP8 linear weight/scale buffers.
    w_nd = wq.transpose(0, 1).contiguous()
    scale = ws.transpose(0, 1).contiguous()
    ...
    print("Casting contiguous FP8 weight to NZ...", flush=True)
    w_nz = torch_npu.npu_format_cast(w_nd, ACL_FORMAT_FRACTAL_NZ, customize_dtype=torch.float8_e4m3fn)
    ...
    print(f"PASS: W8A8 MXFP8 NZ matches ND (rtol={args.rtol}, atol={args.atol})")
