...
def make_inputs(
    ...
    ql_nope = torch.empty((tokens, heads, KV_LORA_RANK), dtype=torch.float32).uniform_(-1.0, 1.0).to(torch.bfloat16)
    q_pe = torch.empty((tokens, heads, ROPE_HEAD_DIM), dtype=torch.bfloat16)

    packed_kv_cache = (
        torch.empty((num_blocks, block_size, 1, cache_dim), dtype=torch.float32).uniform_(-1.0, 1.0).to(torch.bfloat16)
    )
...
def compare(golden: torch.Tensor, actual: torch.Tensor) -> bool:
    ...
    cos = float(torch.nn.functional.cosine_similarity(g.reshape(1, -1), a.reshape(1, -1)).item())
...
def run_batch(n: int, heads: int, block_size: int, base_seed: int) -> int:
    ...
    for i in range(n):
        ...
        if ok:
            ...
            failed += 1
            fail_cases.append(f"T={tokens} N={cur_heads} actual_kv={actual_kv} topk={topk} seed={seed}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--batch", type=int, default=0, help="Run N randomized test cases. 0 means single run with explicit params."
    )
