...
import vllm_ascend.attention.sparse_mla as sparse_mla
...
def test_shared_sfa_nope_metadata_pages_lengths_and_draft_buffers(monkeypatch, block_size, a5):
    ...
    monkeypatch.setattr(torch.ops._C_ascend, "npu_sparse_flash_mla_metadata", plan, raising=False)
...
def test_nope_operator_masks_unwritten_graph_rows(monkeypatch, a5):
    ...
    name = "npu_sparse_flash_mla" if a5 else "npu_sparse_flash_attention"
    monkeypatch.setattr(torch.ops._C_ascend, name, op, raising=False)
...
def test_a5_smla_uses_original_cache_sorted_indices_and_stable_metadata(monkeypatch):
    ...
    monkeypatch.setattr(torch.ops._C_ascend, "npu_sparse_flash_mla_metadata", build, raising=False)
    ...
    monkeypatch.setattr(torch.ops._C_ascend, "npu_sparse_flash_mla", smla, raising=False)
