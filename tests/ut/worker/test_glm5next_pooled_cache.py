        ...
        if request.param == "v2":
            ...
            def reshape(plan, raw):
                ...
                return attn_utils._reshape_kv_cache_v2(
                    groups, raw, "auto", [group.kv_cache_spec.block_size for group in plan.kv_cache_groups], {}, plan
                )
...
def test_mrv2_copy_on_write_preserves_pooled_pages_and_layer_bindings(runner_factory, monkeypatch):
    ...
    def initialize(_runner, cache_config):
...
def test_standalone_mtp_uses_existing_compressed_cache_allocator(runner_factory):
    ...
    assert caches[STATE][0].shape == (3, 2, 3)
...
@pytest.mark.skipif(vllm_version_is("0.28.0"), reason="Descriptor strides are a main API.")
